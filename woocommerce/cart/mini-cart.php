...
<?php if (!WC()->cart->is_empty()) : ?>
	...
	<ul style="max-height: 230px; overflow: scroll;" class="woocommerce-mini-cart cart_list product_list_widget <?php echo esc_attr($args['list_class']); ?>">
		<?php
		...
		?>
				<li class="mb-3 woocommerce-mini-cart-item <?php echo esc_attr(apply_filters('woocommerce_mini_cart_item_class', 'mini_cart_item', $cart_item, $cart_item_key)); ?>">
					<?php
					echo apply_filters( // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
						'woocommerce_cart_item_remove_link',
						sprintf(
							'<a style="z-index:11;" href="%s" class="remove remove_from_cart_button" aria-label="%s" data-product_id="%s" data-cart_item_key="%s" data-product_sku="%s">&times;</a>',
					...
					<div class="quantity">
						...
						<span>x</span>
						<span class="product_price text-danger font-weight-bold"><?php echo $product_price; ?></span>
					</div>
				</li>
		<?php
			}
